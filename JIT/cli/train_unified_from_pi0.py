#!/usr/bin/env python3
"""Train the unified Tube1 policy from frozen pi0 actor parameters.

Only initialization differs from the matched fresh-start config: the observation
normalizer and actor come from frozen pi0.  Critic and optimizer remain fresh.
"""
...
def _load_warm_target_config(path: Path):
    """Reuse the canonical validator while allowing one explicit warm-start shape."""
    path = Path(path)
    raw = _read_json(path)
    ...
    if initialization != expected:
        raise ValueError("pi0 warm-start initialization contract drift")
...
def main() -> int:
    ...
    def warm_trainer(**kwargs):
        ...
        if kwargs.get("restore_value_fn") is not False:
            raise ValueError("pi0 warm-start requires a fresh critic")
        call = dict(kwargs)
