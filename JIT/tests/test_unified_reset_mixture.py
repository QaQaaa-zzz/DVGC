...
ROUND2_CONFIG_SHA256 = "ddb3f407a1c8a7d426d6c1296d94a31f6a3748bae4c5814fe0caf15211b8e94e"
ROUND2_RUN_ID = "pi_unified_round2_natural50_10009600_seed821101_20260831"
...
ROUND1_CHECKPOINT_PAYLOAD_SHA256 = (
    "fb107a5f31b1455f9626c3be68efab36457fb801fcfbba9e99acc0deff3b5719"
)
ROUND1_CANONICAL_TRACE_SHA256 = (
    "1f67dcfd975664da3976945276ef068b5aaace51b1218e95014b2f2b3802c9b5"
)
ROUND1_CANONICAL_PROTOCOL_SHA256 = (
    "df67f326d3963981e85bd9db1d136bbb9587488dab1e21e275329071819906bb"
)
ROUND1_NATURAL_TRANSITION_FRACTION = 0.03477633328064388
...
def test_round2_config_changes_only_reset_distribution_from_round1(jit_root):
    round1 = read_json(jit_root / "configs/pi_unified_round1_natural10.json")
    round2 = read_json(jit_root / "configs/pi_unified_round2_natural50.json")
    config = load_unified_formal_config(
        jit_root / "configs/pi_unified_round2_natural50.json"
    )

    for key in ("inputs", "initialization", "runtime", "ppo", "formal"):
        assert round2[key] == round1[key]
    assert round2["reset_mixture"]["schema"] == RESET_MIXTURE_SCHEMA
    assert config.reset_mixture.selection == "bernoulli_per_episode"
    assert config.reset_mixture.natural_reset_probability == pytest.approx(0.5)
    assert config.reset_mixture.soft_tube_probability == pytest.approx(0.5)
    assert round2["claim_boundary"]["single_variable_change"] == (
        "natural_reset_probability_0.1_to_0.5"
    )
    assert round2["claim_boundary"]["round2_single_variable_iteration"] is True
    assert round2["claim_boundary"]["test_data_used"] is False
    assert round2["claim_boundary"]["validation_data_used"] is False


def test_round2_prelaunch_provenance_and_run_declaration_are_locked(jit_root):
    path = jit_root / "configs/pi_unified_round2_natural50.json"
    raw = read_json(path)
    config = load_unified_formal_config(path)
    boundary = raw["claim_boundary"]
    declaration = raw["run_declaration"]

    assert config.config_sha256 == ROUND2_CONFIG_SHA256
    assert boundary["round1_canonical_gate"] == "CANONICAL_FULL_RECOVERY_FAIL"
    assert boundary["round1_failure_stage"] == "pre_jump"
    assert boundary["round1_terminal_reason"] == "yaw_limit"
    assert boundary["round1_checkpoint_payload_sha256"] == (
        ROUND1_CHECKPOINT_PAYLOAD_SHA256
    )
    assert boundary["round1_canonical_trace_npz_sha256"] == (
        ROUND1_CANONICAL_TRACE_SHA256
    )
    assert boundary["round1_canonical_protocol_sha256"] == (
        ROUND1_CANONICAL_PROTOCOL_SHA256
    )
    assert boundary["round1_logged_natural_transition_fraction"] == pytest.approx(
        ROUND1_NATURAL_TRANSITION_FRACTION, rel=0.0, abs=1.0e-15
    )
    assert declaration == {
        "run_id": ROUND2_RUN_ID,
        "output_dir": f"JIT/runs/pi_unified/{ROUND2_RUN_ID}",
        "purpose": "round2_single_variable_natural50_soft_tube50_unified_training",
        "status": "predeclared_not_started",
    }


def test_generic_mixed_reset_rejects_invalid_probability_contract(jit_root, tmp_path):
    payload = read_json(jit_root / "configs/pi_unified_round2_natural50.json")
    drifted = deepcopy(payload)
    drifted["reset_mixture"]["natural_reset_probability"] = 0.6
    drifted["reset_mixture"]["soft_tube_probability"] = 0.5
    path = tmp_path / "invalid_sum.json"
    path.write_text(json.dumps(drifted), encoding="utf-8")
