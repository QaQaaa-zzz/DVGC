...
def test_dense_collector_saves_multiple_real_frames_and_exact_action_prefixes(tmp_path,monkeypatch):
    ...
    def state(tick):
        ...
        return SimpleNamespace(data=SimpleNamespace(qpos=jp.array([x,0.,.2]),qvel=jp.array([2.,0.,-1. if tick>=5 else 1.])),
            obs={'state':jp.array([float(tick)])},done=jp.array(tick>=9),
            info={'reset_from_soft_tube':jp.array(False),'reset_from_jump_start':jp.array(True),'expert_switching_used':jp.array(False),
                  ...
                  'down_events':SimpleNamespace(valid_contact_seen=jp.array(False))})
    ...
    result=c.collect_jump_start_connected_candidates([{'phase':'upstream','x_target_m':2.8,'proposal_family_index':0,
        ...
        sampling_mode='trajectory_slices_v2',slice_spacing_m=.05,max_candidates_per_attempt=32)
    rows=result['entries']
    assert len(rows)>3 and {r['phase'] for r in rows}=={'upstream','downstream'}
    assert result['environment_interactions']==9
    for r in rows:
    ...
    assert rows[-1]['jump_start_reachability']['unperturbed_suffix_transitions']>0
...
def test_supervisor_falls_back_preserves_cost_and_resumes(tmp_path,monkeypatch):
    from jit_dvgc import dense_tube as d
    ...
    result=d.run(tmp_path,output)
    assert result['status']=='completed'
    assert result['backend_decisions']['pi_1']['backend']=='serial'
    assert result['backend_decisions']['pi_0']['backend']=='device'
    assert result['charged_interactions']==7*3+6400+5+4*7
    count=len(calls)
    assert d.run(tmp_path,output)['status']=='completed' and len(calls)==count
    write(output/'figures/figure_metadata.json',{'changed':True})
    assert d.run(tmp_path,output)['status']=='engineering_error'
