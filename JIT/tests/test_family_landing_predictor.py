...
def test_lock_forward_scores_reads_catalog_snapshots_without_labels(
    ...
    monkeypatch.setattr(predictor, "physical_state_sha256", lambda _: "state-up")
    monkeypatch.setattr(
        predictor.iterative,
        "_score",
...
def test_evaluate_locked_forward_scores_joins_only_after_labels(
    ...
    locked = json.loads(score_path.read_text())
    locked.update(catalog_file_sha256="catalog", catalog_protocol_sha256="protocol")
    locked["scores_sha256"] = predictor.canonical_sha256(locked)
    score_path.write_text(json.dumps(locked))
    rows = (
    ...
    rows = tuple({**row, "acquisition_protocol_sha256": "protocol"} for row in rows)
    monkeypatch.setattr(
        predictor.iterative,
        "_load_role",
        lambda _root, _role: ({"role_manifest_sha256": "role-manifest", "source_acquisition_catalog_sha256": "catalog"}, rows),
    )
...
def test_tied_ap_is_order_invariant():
    for targets in ([1, 0], [0, 1]):
        assert predictor._ranking_metrics(np.array(targets), np.array([.5, .5]))["average_precision"] == .5


def test_single_class_metrics_are_serializable_without_inventing_negatives():
    metrics = predictor._ranking_metrics(np.array([1, 1]), np.array([.2, .8]))
    assert metrics["roc_auc"] is None
    assert metrics["average_precision"] == 1.0
    json.dumps(metrics, allow_nan=False)


def test_tampered_score_lock_fails_before_loading_labels(tmp_path, monkeypatch):
    import pytest
    locked = {"schema": "jit_policy_family_landing_forward_scores_v1",
              "status": "locked_before_outcome_analysis", "outcome_labels_read": False,
              "role": "acceptance", "entries": [{"score": .2}]}
    locked["scores_sha256"] = predictor.canonical_sha256(locked)
    locked["entries"][0]["score"] = .9
    path = tmp_path / "scores.json"
    path.write_text(json.dumps(locked))
    def forbidden(*args):
        raise AssertionError("labels loaded before score integrity verification")
    monkeypatch.setattr(predictor.iterative, "_load_role", forbidden)
    with pytest.raises(ValueError, match="scores_sha256 drift"):
        predictor.evaluate_locked_forward_scores(scores_path=path, role_root=tmp_path,
            role="acceptance", output_path=tmp_path / "audit.json")


def test_fitting_does_not_read_acceptance_support():
    phase = {"positive_count": 30, "negative_count": 20, "parent_group_count": 5}
    counts = {role: {"upstream": phase, "downstream": phase} for role in ("train", "calibration")}
    assert predictor.assess_phase_support(counts)["upstream"]["fit_authorized"]


def test_fresh_audit_accepts_single_class_and_rejects_target_drift(tmp_path, monkeypatch):
    import pytest
    test_evaluate_locked_forward_scores_joins_only_after_labels(tmp_path, monkeypatch)
    original = predictor.iterative._load_role
    manifest, rows = original(tmp_path, "acceptance")
    monkeypatch.setattr(predictor.iterative, "_load_role", lambda *args: (
        manifest, tuple({**row, "label": 1} for row in rows)))
    result = predictor.evaluate_locked_forward_scores(scores_path=tmp_path / "scores.json",
        role_root=tmp_path, role="acceptance", output_path=tmp_path / "single-class.json")
    assert result["negative_count"] == 0
    assert result["false_positive_rate_at_locked_threshold"] is None
    locked = json.loads((tmp_path / "scores.json").read_text())
    locked.pop("scores_sha256")
    locked["target_policy_family_sha256"] = "f" * 64
    locked["scores_sha256"] = predictor.canonical_sha256(locked)
    (tmp_path / "scores.json").write_text(json.dumps(locked))
    with pytest.raises(ValueError, match="target policy family"):
        predictor.evaluate_locked_forward_scores(scores_path=tmp_path / "scores.json",
            role_root=tmp_path, role="acceptance", output_path=tmp_path / "wrong-bank.json")
