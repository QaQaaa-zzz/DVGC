...
def coverage_rows(old_points,old_report,new_points,new_report):
    ...
    for name in [*new_report['policy_names'],'union']:
        ...
        for field in ('root_cell','full_cell'):
            before={p[field] for p,ok in zip(old_points,old_report['masks'][name],strict=True) if ok}
            after={p[field] for p,ok in zip(new_points,new_report['masks'][name],strict=True) if ok}
            row.update({field+'_old':len(before),field+'_new_panel':len(after),
                        field+'_novel':len(after-before),field+'_cumulative':len(before|after)})
        rows.append(row)
    return rows
...
def compare_coverage(old_points,old_report,new_points,new_report,output):
    ...
    for name,axs in zip(names,axes):
        old=[p for p,ok in zip(old_points,old_report['masks'][name]) if ok]
        ...
        for ax,y,label in zip(axs,('root_z_m','root_vz_mps'),('z (m)','vz (m/s)')):
            for points,color,title in ((old,'#aaaaaa','Previous witnessed support'),(new,'#009E73','New root-state cells')):
            ...
            ax.set(xlabel='x (m)',ylabel=label,title=name.replace('pi_','π'));ax.grid(alpha=.15)
