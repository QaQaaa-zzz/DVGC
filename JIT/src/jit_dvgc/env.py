...
class TwoPhaseBikeEnv(mjx_env.MjxEnv):
    ...
    def _reset_descent_from_sample(self, sample):
        ...
        metrics.update({
            ...
            "reward/descent_pitch_rate": jp.asarray(0., jp.float32),
            "reward/descent_action_smoothness": jp.asarray(0., jp.float32),
    ...
    def _step_descent(self, state, action):
        model = self._require_runtime_model(); index = self._bundle.model_index
        a = jp.clip(jp.asarray(action, jp.float32), -1., 1.)
        ctrl = map_action(a, state.data.qpos[index.knee_qpos_address], self._bundle.action_mapping)
        data = mjx_env.step(model, state.data, ctrl, self.n_substeps)
        geometry = extract_geometry(data, self._geometry); rs = self._reward_state(data, geometry); prev = state.info["events"]
        signals = DescentSignals(data.qpos[index.root_qpos_address], geometry.front_wheel_terrain_clearance, geometry.rear_wheel_terrain_clearance, geometry.maximum_wheel_penetration, geometry.prohibited_contact, jp.isfinite(data.qpos).all() & jp.isfinite(data.qvel).all() & jp.isfinite(a).all(), geometry.roll, geometry.pitch, data.qpos[index.root_qpos_address] < state.info["reward_state"].x - self._resolved_config.physical_limits.max_backward_distance, forward_velocity=data.qvel[index.root_dof_address])
        events = advance_descent_events(prev, signals, self._resolved_config.descent)
        terminal = classify_descent_terminal(signals, events, self._resolved_config.descent, self._resolved_config.physical_limits, state.info["episode_step"] + 1, self._resolved_config.ppo.episode_horizon)
        reward_input = DescentRewardInputs(rs.x - state.info["reward_state"].x, events.valid_contact_seen, prev.valid_contact_seen, (events.post_contact_ticks > prev.post_contact_ticks) if self._resolved_config.descent.continuous_stability else events.valid_contact_seen, events.recovery_success, prev.recovery_success, signals.body_contact, terminal.physical_failure, terminal.truncated, geometry.roll, geometry.pitch, data.qvel[index.root_dof_address + 3], data.qvel[index.root_dof_address + 4], a, state.info["last_action"])
        rr = descent_recovery_reward(reward_input, self._resolved_config.descent)
        og = self._observable_geometry(data, geometry); frame = observable_frame(data, index, og, a, jp.asarray(True)); history = advance_history(state.info["history"], frame); actor = actor_observation(history, jp.asarray(False)); critic = privileged_observation(data, actor, og)
        info = {**state.info, "history": history, "events": events, "last_action": a, "reward_state": rs, "terminated": terminal.terminated, "truncated": terminal.truncated, "time_out": terminal.truncated.astype(jp.float32), "end_code": terminal.end_code, "success": terminal.success, "physical_failure": terminal.physical_failure, "timeout": terminal.truncated, "episode_step": state.info["episode_step"] + 1, "episode_return": state.info["episode_return"] + rr.total}
        metrics = {**self._zero_metrics(), "reward": rr.total, **{f"reward/descent_{k}": v for k, v in rr.components.__dict__.items()}, "event/descent_airborne_seen": events.airborne_seen.astype(jp.float32), "event/descent_valid_contact_seen": events.valid_contact_seen.astype(jp.float32), "event/descent_post_contact_ticks": events.post_contact_ticks.astype(jp.float32), "terminal/descent_success": terminal.success.astype(jp.float32), "terminal/descent_physical_failure": terminal.physical_failure.astype(jp.float32), "terminal/descent_timeout": terminal.truncated.astype(jp.float32)}
