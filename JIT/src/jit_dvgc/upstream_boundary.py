...
def action_basis_directions(action_order: Sequence[str] = ACTION_ORDER) -> tuple[dict[str, Any], ...]:
    result = []
    for dimension, name in enumerate(action_order):
        for sign in (-1, 1):
            vector = [0.0] * len(action_order)
            vector[dimension] = float(sign)
            result.append({"action_dimension": dimension, "action_name": str(name), "sign": sign, "basis_vector": vector})
    return tuple(result)
...
def row_identity(row: Mapping[str, Any]) -> tuple[Any, ...]:
    return (str(row.get("source_bank", "")), str(row.get("parent_group_id", "")), int(row.get("seed", -1)), str(row.get("role", "")), int(row.get("tick", -1)), str(row.get("state_sha256", "")))
...
def label_reasons(row: Mapping[str, Any]) -> tuple[str, ...]:
    return tuple(sorted({str(branch.get("reason", "")) for branch in row.get("branches", [])}))
...
def snapshot_distance(a: HandoffSnapshot, b: HandoffSnapshot) -> dict[str, float | bool]:
    ...
    return {
        ...
        "exact": bool(np.array_equal(a.qpos, b.qpos) and np.array_equal(a.qvel, b.qvel) and np.array_equal(a.observation, b.observation)),
    }


def are_near_duplicates(a: HandoffSnapshot, b: HandoffSnapshot, *, qpos_atol: float = DEFAULT_NEAR_ATOL, qvel_atol: float = DEFAULT_NEAR_ATOL, observation_atol: float = DEFAULT_NEAR_ATOL) -> bool:
    d = snapshot_distance(a, b)
    return bool(d["qpos_linf"] <= qpos_atol and d["qvel_linf"] <= qvel_atol and d["observation_linf"] <= observation_atol)
...
class AuditedAnchor:
    ...
    def sort_key(self) -> tuple[Any, ...]:
        return (int(self.row["seed"]), str(self.row["parent_group_id"]), int(self.row["tick"]), str(self.row.get("state_sha256", "")))


def _cluster_near_duplicates(anchors: Sequence[AuditedAnchor], *, qpos_atol: float, qvel_atol: float, observation_atol: float) -> list[list[AuditedAnchor]]:
    clusters: list[list[AuditedAnchor]] = []
    for anchor in sorted(anchors, key=lambda x: x.sort_key):
        for cluster in clusters:
            if are_near_duplicates(anchor.snapshot, cluster[0].snapshot, qpos_atol=qpos_atol, qvel_atol=qvel_atol, observation_atol=observation_atol):
    ...
    *,
    ...
    for raw in _catalog_payload(catalog)["entries"]:
        ...
        if seed not in train or str(label.get("split")) != "train":
            continue
        joined.append(AuditedAnchor(row, label, load_snapshot(snapshot_path(catalog, row)), "nominal"))

    negatives = [x for x in joined if str(x.row.get("role")) == negative_role and int(x.label.get("success_count", 0)) == 0 and failure_reason in label_reasons(x.label)]
    if not negatives:
        raise ValueError("no matching TRAIN failure anchors found")
    clusters = _cluster_near_duplicates(negatives, qpos_atol=qpos_atol, qvel_atol=qvel_atol, observation_atol=observation_atol)
    representatives = [cluster[0] for cluster in clusters][:max_negative_anchors]
    selected = [AuditedAnchor(x.row, x.label, x.snapshot, "failure_anchor") for x in representatives]

    neighbor_outcomes = []
    for failure_anchor in tuple(selected):
        for role in guard_roles:
            matches = sorted((x for x in joined if x.row["parent_group_id"] == failure_anchor.row["parent_group_id"] and x.row["role"] == role), key=lambda x: x.sort_key)
            if not matches:
                neighbor_outcomes.append({"parent_group_id": failure_anchor.row["parent_group_id"], "role": str(role), "present": False})
                continue
            item = matches[0]
            positive = int(item.label.get("success_count", 0)) == int(item.label.get("branch_count", 1))
            neighbor_outcomes.append({"parent_group_id": item.row["parent_group_id"], "seed": int(item.row["seed"]), "role": item.row["role"], "present": True, "positive": positive, "reasons": list(label_reasons(item.label)), "state_sha256": item.row.get("state_sha256")})
            if positive:
                selected.append(AuditedAnchor(item.row, item.label, item.snapshot, "positive_guard"))

    ordered = sorted(negatives, key=lambda x: x.sort_key)
    pairwise = []
    for i, left in enumerate(ordered):
        for right in ordered[i + 1:]:
            pairwise.append({"left_seed": int(left.row["seed"]), "right_seed": int(right.row["seed"]), "left_state_sha256": left.row.get("state_sha256"), "right_state_sha256": right.row.get("state_sha256"), **snapshot_distance(left.snapshot, right.snapshot), "near_duplicate": are_near_duplicates(left.snapshot, right.snapshot, qpos_atol=qpos_atol, qvel_atol=qvel_atol, observation_atol=observation_atol)})
    ...
    audit = {
        ...
        "near_duplicate_tolerances": {"qpos_atol": qpos_atol, "qvel_atol": qvel_atol, "observation_atol": observation_atol},
        "near_duplicate_cluster_count": len(clusters),
        "near_duplicate_clusters": [{"representative_seed": int(c[0].row["seed"]), "representative_state_sha256": c[0].row.get("state_sha256"), "member_seeds": [int(x.row["seed"]) for x in c], "member_state_sha256": [x.row.get("state_sha256") for x in c]} for c in clusters],
        "pairwise_distances": pairwise,
        "selected_failure_anchors": [{"seed": int(x.row["seed"]), "parent_group_id": x.row["parent_group_id"], "role": x.row["role"], "tick": int(x.row["tick"]), "state_sha256": x.row.get("state_sha256"), "reasons": list(label_reasons(x.label))} for x in selected if x.anchor_kind == "failure_anchor"],
...
def collect_reachable_boundary_candidates(
    ...
    *,
    ...
    strengths, durations = validate_strengths(strengths), validate_durations(durations)
    if not anchors:
    ...
    protocol_payload = {**dict(protocol), "schema": "jit_upstream_boundary_protocol_v1", "split": "train", "action_order": list(ACTION_ORDER), "direction_family": "action_basis", "strengths": list(strengths), "durations": list(durations), "training_transitions": 0}
    ...
    (output_dir / "protocol.json").write_text(json.dumps({**protocol_payload, "protocol_sha256": protocol_hash}, indent=2, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
        ...
        for duration in durations:
            for strength in strengths:
                for direction in action_basis_directions():
                    ...
                    entries.append({
                        "snapshot": str(relative), "source_bank": bank_name, "candidate_kind": "reachable_boundary",
                        "parent_group_id": str(row["parent_group_id"]), "parent_trajectory": str(row["parent_group_id"]), "seed": int(row["seed"]), "role": str(row["role"]), "tick": int(snapshot.tick), "state_sha256": state_hash, "result_state_sha256": state_hash,
                        "anchor_kind": anchor.anchor_kind, "anchor_source_bank": str(row["source_bank"]), "anchor_snapshot": str(row["snapshot"]), "anchor_parent_group_id": str(row["parent_group_id"]), "anchor_seed": int(row["seed"]), "anchor_role": str(row["role"]), "anchor_tick": int(row["tick"]), "anchor_state_sha256": str(row.get("state_sha256", "")), "anchor_source_checkpoint": str(row.get("source_checkpoint", "")), "anchor_source_training_transitions": int(row.get("source_training_transitions", -1)), "anchor_source_actor_sha256": str(row.get("source_actor_sha256", "")), "anchor_split": "train",
                        "perturbation": {**direction, "strength": float(strength), "duration": int(duration), "variant_index": current_variant, "nominal_actions": nominal_actions, "perturbed_actions": perturbed_actions, "effective_deltas": effective_deltas},
                        "protocol_seed": int(protocol_payload.get("protocol_seed", 0)), "protocol_sha256": protocol_hash,
                        "z": float(qpos[2]), "vx": float(qvel[0]), "vz": float(qvel[2]), "pitch": float(qpos[4]) if qpos.size > 4 else 0.0, "pitch_rate": float(qvel[4]) if qvel.size > 4 else 0.0,
                    })

    report = {"schema": BOUNDARY_CATALOG_SCHEMA, "status": "completed", "target": "V_up", "selection": "train_unique_reachable_boundary_action_basis", "split": "train", "protocol_sha256": protocol_hash, "training_transitions": 0, "anchor_count": len(anchors), "attempted_candidate_count": attempted, "candidate_count": len(entries), "collection_transitions": transitions, "exclusion_counts": exclusions, "source_bank": bank_name, "entries": entries}
