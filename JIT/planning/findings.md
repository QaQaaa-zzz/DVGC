...
| Issue | Resolution |
|---|---|
| The first committed refinement run exhausted GPU allocation while duration 23 labeling was 113/120; the function persisted only failure counts, not partial rows | Treat the 113 rows as unusable, preserve/count all 998 failed-attempt interactions, reuse only the complete acquisition, and recover into a new label-attempt directory under an explicit old-HEAD-to-repair-HEAD record. |
| Acquisition and continuation helpers created new `jax.jit` wrappers for the same bound reset/step methods at every duration | Reuse one search-lifetime pair of compiled callables so later durations do not retain redundant executable/cache state. |
| The full contiguous duration 17--32 refinement yielded 1,909 new downstream labels and every one was positive, including all 129 terminal-clipped candidates at durations 31--32 | Duration-only local action-basis refinement does not bracket the negative side of pi_0 continuation feasibility; stop at the declared grid and make a new explicit TRAIN-only acquisition-method decision before any continuation-field fitting. |
| The symmetric higher-strength panel found exactly five negatives, all at hip positive-sign strength 0.30 and spanning all five parent groups; every other combination stayed positive | The negative direction is now evidence-localized. Run at most one targeted 25-attempt hip-positive completion panel, retain physical deduplication, and stop strength-only acquisition if it cannot add the remaining 15 negatives. |
