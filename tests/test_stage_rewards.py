...
def test_event_dominates_bounded_dense_stage_reward():
 ...
 x=compute_stage_next_entry_reward(cfg=cfg,objective='apex_to_descent',feature=f,previous_feature=f,action=a,previous_action=a,next_entry=jp.asarray(True),hard_failure=jp.asarray(False))
...
def test_ascent_rewards_upward_height_progress():
 ...
 def r(f):return compute_stage_next_entry_reward(cfg=cfg,objective='ascent_to_apex',feature=f,previous_feature=p,action=a,previous_action=a,next_entry=jp.asarray(False),hard_failure=jp.asarray(False))['reward']
