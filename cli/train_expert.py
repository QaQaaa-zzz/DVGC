...
def main():
    ...
    run.mkdir(parents=True); blocks=run/"blocks"; blocks.mkdir(); rows=[]; metrics={"status":"preflight","stage":a.stage,"curriculum":a.curriculum,"seed":a.seed,"effective_steps":102400,"block_steps":25600,"gate_mode":a.gate_mode,"entry_set_sha256":file_sha256(a.entry_set),"candidate_bank_sha256":file_sha256(a.bank),"initial_policy_hash":initial_spec.policy_hash,"landing_policy_hash_before":landing_hash_before,"landing_retention_required":False,"progress":rows}; save_json(run/"training_metrics.json",metrics)
