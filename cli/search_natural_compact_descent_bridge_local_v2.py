"""Move the bounded natural bridge control window into approach/Takeoff tail."""
...
from cli.search_natural_compact_descent_bridge_local_v1 import CHANNELS,latin_hypercube
...
PRIOR=Path("runs/natural_compact_descent_bridge_v1/local_residual_search_v1/NATURAL_COMPACT_DESCENT_LOCAL_SEARCH_V1_REPORT.json")
DEFAULT_RUN=Path("runs/natural_compact_descent_bridge_v1/local_residual_search_v2_early_window")
SEED=3_520_000_000

def score(row):
    return (int(row["final_recovery"]),int(row["landing"]),int(not row["early_failure"]),
            int(row["minimum_pitch_margin"]>=0),-float(row["minimum_distance"]),
            int(row["stable_descent_ticks"]),float(row["minimum_pitch_margin"]))

def selected_dimensions(pilot,baseline,count=4):
    ranked=[]
    for dimension in range(8):
        pair=[row for row in pilot if row["perturbed_dimension"]==dimension]
        ranked.append((max([score(baseline)]+[score(row) for row in pair]),dimension))
    return [d for _,d in sorted(ranked,reverse=True)[:count]]
...
def main():
    ...
    if prior["status"]!="PROGRESS" or prior["next"]!="bounded_trust_region_round_2":raise SystemExit("round-1 prerequisite mismatch")
    ...
    late=np.asarray(prior["top"]["params"],np.float32);artifact=pickle.loads((EXPERT/"adapter.pkl").read_bytes());tube=SnapshotBank.load(TUBE)
    ...
    center,scale=robust_normalization(features,cfg.descent_entry_scale_floors);switch_tick=16
    ...
    def run(params):
        ...
        while int(state.info["phase"])<1 and not float(state.done):
            ...
            if 10<=approach<16:action=np.clip(action+params[0],-1,1)
            state=step(state,action);approach+=1
        positive=float(state.data.qvel[2])>0;previous=float(state.data.qvel[2]);apex=None;landing=False;stable=0;minimum=float("inf");margin=float("inf");closest=None
        for tick in range(220):
            if tick<switch_tick:
                action=_reference_action(env,cfg,state,reference,tick,3,105,10)
                if tick<8:action=np.clip(action+params[1],-1,1)
                elif tick<16:action=np.clip(action+late[min((tick-8)//4,1)],-1,1)
            ...
            distance,index,_=normalized_nearest(feature,features,center,scale);minimum=min(minimum,distance);margin=min(margin,np.deg2rad(float(cfg.max_pitch_deg))-abs(float(physical[4])))
        ...
        return {"params":params.tolist(),"approach_ticks":approach,"apex_tick":apex,"early_failure":bool(float(state.done) and stable==0),
            ...
            "minimum_distance":minimum,"minimum_pitch_margin":margin,"closest":closest,"termination_reason":END_REASON.get(int(state.info["end_code"]),"unknown")}
    ...
    save_json(root/"manifest.json",{"status":"FROZEN_BEFORE_OUTCOMES","inputs":inputs,"approach_tail_ticks":[10,16],"early_local_ticks":[0,8],
        "fixed_late_local_ticks":[8,16],"fixed_late_params":late.tolist(),"switch_tick":switch_tick,"pilot_epsilon":.06,"expansion_bound":.14,
        "pilot":"center plus both signs in 8 dimensions","expansion":"64-point LHS in four pilot-selected dimensions","PPO_authorization":False})
    ...
    for dim in range(8):
        for sign in (-1,1):
            x=zero.reshape(-1).copy();x[dim]=sign*.06;row=run(x.reshape(2,4));row.update({"perturbed_dimension":dim,"sign":sign});pilot.append(row)
    dims=selected_dimensions(pilot,baseline);best_pilot=max([baseline]+pilot,key=score);improved=score(best_pilot)>score(baseline)
    save_json(root/"finite_difference_pilot.json",{"baseline":baseline,"rows":pilot,"selected_dimensions":dims,"improved":improved})
    expansion=[]
    if improved:
        for sample in latin_hypercube(SEED+100,64,len(dims),.14):
            x=zero.reshape(-1).copy();x[dims]=sample;expansion.append(run(x.reshape(2,4)))
    top=max([baseline]+pilot+expansion,key=score);a1=run(top["params"]);a2=run(top["params"]);exact=top==a1==a2
    save_json(root/"search_results.json",{"baseline":baseline,"pilot":pilot,"selected_dimensions":dims,"expansion":expansion,"top":top,"replay_a":a1,"replay_b":a2})
    progress=top["minimum_distance"]<baseline["minimum_distance"]-1e-3 or top["stable_descent_ticks"]>baseline["stable_descent_ticks"] or top["minimum_pitch_margin"]>baseline["minimum_pitch_margin"]+.005
    report={"status":"PASS" if top["final_recovery"] and exact else ("PROGRESS" if progress and exact else "FAIL"),"head":subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),
        "pilot_rollouts":17,"expansion_rollouts":len(expansion),"selected_dimensions":[{"segment":"approach_tail" if d<4 else "early_takeoff","channel":CHANNELS[d%4]} for d in dims],
        "baseline":baseline,"top":top,"distance_improvement":baseline["minimum_distance"]-top["minimum_distance"],"stable_tick_gain":top["stable_descent_ticks"]-baseline["stable_descent_ticks"],
        "pitch_margin_gain":top["minimum_pitch_margin"]-baseline["minimum_pitch_margin"],"exact_replay_twice":exact,"PPO_authorization":False,
        "next":"teacher_neighborhood_audit" if top["final_recovery"] else ("bounded_early_window_round_3" if progress else "bounded_local_budget_exhausted")}
    save_json(root/"NATURAL_COMPACT_DESCENT_LOCAL_SEARCH_V2_REPORT.json",report);save_json(root/"completed.json",{"status":report["status"],"next":report["next"]})
    print(json.dumps({k:report[k] for k in ("status","selected_dimensions","distance_improvement","stable_tick_gain","pitch_margin_gain","exact_replay_twice","top","next")},indent=2))
