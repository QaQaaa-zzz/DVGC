...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--run',default=str(DEFAULT_RUN));p.add_argument('--source',default=str(SOURCE));p.add_argument('--base',default=str(BASE));p.add_argument('--seed',type=int,default=SEED);p.add_argument('--namespace',default=NAMESPACE);a=p.parse_args();root=Path(a.run);source_path=Path(a.source);base_path=Path(a.base);seed_base=int(a.seed);namespace=str(a.namespace)
 ...
 source=SnapshotBank.load(source_path);base_bank=SnapshotBank.load(base_path);artifact=pickle.loads((EXPERT/'adapter.pkl').read_bytes());dp,_,_=load_bundle(PI_D,verify_files=True);lp,_,_=load_bundle(PI_L,verify_files=True)
 ...
 inputs={'candidate_sha256':file_sha256(source_path),'base_tube_sha256':file_sha256(base_path),'adapter_sha256':file_sha256(EXPERT/'adapter.pkl'),'policy_identity_hash':artifact['policy_identity_hash'],'C_L':file_sha256(C_L),'xml':EXPECTED['xml'],'seed':seed_base,'namespace':namespace}
 ...
 for state_index,record in enumerate(source.records):
  ...
  for variant_index,(variant,env,rollout) in enumerate(variants):
   ...
   for b in indices:
    seed=branch_seed(seed_base,state_index,b);seeds.append(seed);deltas.append(np.random.default_rng(seed).uniform(-.02,.02,2).astype(np.float32))
   batch=_perturb_batch(env,record,seeds,deltas);raw_out=jax.device_get(rollout(batch,jnp.zeros((len(seeds),2,4),jnp.float32),jax.random.PRNGKey(branch_seed(seed_base,state_index,variant_index))))
 ...
 new=[];version='descent-compact-'+hashlib.sha256((file_sha256(base_path)+file_sha256(source_path)+namespace).encode()).hexdigest()[:12]
 if passed:
  ...
  combined=union_records(base_bank.records,new);metadata=copy.deepcopy(base_bank.metadata);metadata.update({'last_tube_version':version,'last_policy_version':artifact['policy_identity_hash'],'supersedes':str(base_path.resolve()),'extension_audit_namespace':namespace,'extension_source_sha256':file_sha256(source_path),'branches_per_extension_state':BRANCHES,'continuous_matcher_active':False});SnapshotBank(combined,metadata).save(root/'descent_tube_v4.pkl');handoff=copy.deepcopy(metadata);handoff.update({'entry_bank_role':'canonical_descent_exact_handoff_set','membership_type':'exact_snapshot_identity','radius':None});SnapshotBank(copy.deepcopy(combined),handoff).save(root/'canonical_descent_exact_entry_v3.pkl')
 report={'status':'PASS' if passed else 'FAIL','head':subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),'states':len(rows),'safe_states':sum(r['label']=='safe' for r in rows),'rows':rows,'terminal_summary':summary,'base_states':len(base_bank.records),'combined_states':len(base_bank.records)+len(new),'tube_version':version if passed else None,'tube_path':str(root/'descent_tube_v4.pkl') if passed else None,'tube_sha256':file_sha256(root/'descent_tube_v4.pkl') if passed else None,'exact_handoff_sha256':file_sha256(root/'canonical_descent_exact_entry_v3.pkl') if passed else None,'PPO_authorization':False,'next':'local_entry_reconstruction_against_tube_v4' if passed else 'targeted_extension_audit_failure'}
