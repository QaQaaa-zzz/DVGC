...
def _source_ratios(row,prefix):
    values={name:float(row.get(prefix+name,0.0)) for name in ("flight_curriculum","canonical_entry_rehearsal","landing_tube_rehearsal","natural")}
    total=sum(values.values())
    return {key:(value/total if total>0 else 0.0) for key,value in values.items()}
...
def main():
    ...
    source_rows=[row for row in rows if any(key.startswith("training/reset/") for key in row)]; source_last=source_rows[-1] if source_rows else {}
    ...
    report={
        ...
        "reset_source_distribution":{"candidate_bank":1.0-natural,"natural_start":natural,"candidate_records":bank_count,"protocol":payload.get("reset_protocol"),"training_episode_ratio":_source_ratios(source_last,"training/reset/episode/"),"training_transition_ratio":_source_ratios(source_last,"training/reset/transition/"),"evaluation_episode_ratio":_source_ratios(last,"eval/episode_reset/episode/"),"evaluation_transition_ratio":_source_ratios(last,"eval/episode_reset/transition/")},
