...
def main():
 ...
 def grouped(kind):
  ...
  return {**metrics(target,pred),'high_reach':classification(target,pred)}
 payload={'status':'PASS','artifact_role':'stage_reachability_model_pilot','stage':'takeoff','target':'next_stage_reach_probability','cpu_only':True,'unique_states':len(rows),'unique_parents':len(parents),'train_parents':len(set(x[2] for x in train)),'heldout_parents':len(hold),'split':'parent-held-out 75/25','seed':a.seed,'features':'16D physical feature only','forbidden_inputs_excluded':['reward','success_label_as_input','controller_id','oracle_phase','teacher_id','candidate_kind','reference_index','policy_hash'],'train':metrics([x[1]['p_next'] for x in train],ptr),'heldout':metrics([x[1]['p_next'] for x in test],pte),'leave_one_parent_out':{'overall':{**metrics([x[1]['p_next'] for x in rows],lopo),'high_reach':classification([x[1]['p_next'] for x in rows],lopo)},'canonical_compressed':grouped('canonical_compressed'),'reference_aligned_compressed':grouped('reference_aligned_compressed')},'scope_limitation':'no all-controller-fail states; conditional proposal-support model only','data':{'bank_sha256':file_sha256(a.bank),'labels_sha256':file_sha256(a.labels)}}
