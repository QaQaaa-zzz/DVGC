...
def main():
    ...
    rows_for_source=source.records_for_phase("flight",include_training_only=False);current_policy_hash=file_sha256(Path(a.descent_policy)/"params.pkl")
    try:
        source_policy_hashes=validate_snapshot_source_records(rows_for_source,source.metadata)
        supplied=list(a.candidate_source_policy)
        if not supplied and source_policy_hashes==(current_policy_hash,):supplied=[a.descent_policy]
        verify_source_policy_paths(source_policy_hashes,[str(Path(path)/"params.pkl") for path in supplied],file_sha256)
    ...
    all_rows=rows_for_source; end=len(all_rows) if a.end_index is None else a.end_index
