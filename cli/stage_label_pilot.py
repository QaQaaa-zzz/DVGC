...
def sample_from_state(env,state,previous_vz):
 ...
 snap.update({
  ...
  "dual_wheel_airborne":bool(int(np.asarray(jax.device_get(state.info['had_airborne'])))),
  ...
  "physical_failure":bool(int(np.asarray(jax.device_get(state.info['terminated'])))),
  "nonfinite":not np.isfinite(snap['physical_feature']).all(),
 })
 return snap

def run_branch(env,step,inference,row,stage,seed,horizon,noise):
 ...
 for tick in range(1,horizon+1):
  ...
  state=step(state,action);sample=sample_from_state(env,state,previous_vz);entry=evaluate_entry(stage,sample,env._config)
...
def main():
 ...
 for stage,rows in selected.items():
  phase=CANONICAL_PHASE[stage];cfg=load_config(a.config,{"training_stage":phase,"use_bank_resets":False,"domain_randomization":False,"obs_noise_enable":False});protocol=protocol or protocol_payload(cfg);env=OrangeBikeDVGC(cfg,snapshot_bank=SnapshotBank());step=jax.jit(env.step);policy=a.landing_policy if stage=='landing' else a.flight_policy;inference=build_inference(env,load_params(Path(policy)/'params.pkl'),deterministic=True);envs[stage]=env
  for i,row in enumerate(rows):
   branch_records=[];successes=0
   for branch in range(a.branches):
    seed=STAGE_SEED[stage]+i*100+branch;success,tick,snapshot,quality,reason=run_branch(env,step,inference,row,stage,seed,a.horizon,a.action_noise);successes+=int(success);terminal[f'{stage}:{reason}']+=1;entry_id=None
    if snapshot is not None:
     entry_id=hashlib.sha256(f'{stage}:{row["id"]}:{seed}:{tick}'.encode()).hexdigest()[:32];snapshot.update({"id":entry_id,"candidate_kind":"stage_entry_snapshot","entry_from_stage":stage,"entry_to_stage":{"takeoff":"ascent","ascent":"apex","apex":"descent","descent":"landing","landing":"stable"}[stage],"trajectory_parent_id":row.get('trajectory_parent_id',row['id']),"controller_id":file_sha256(Path(policy)/'params.pkl'),"rollout_seed":seed,"time_to_next_stage":tick,"entry_quality":quality});entries.append(snapshot)
    branch_records.append({"branch_index":branch,"seed":seed,"controller_id":file_sha256(Path(policy)/'params.pkl'),"success":success,"time_to_next_stage":tick,"entry_snapshot_id":entry_id,"entry_quality":quality,"failure_reason":None if success else reason})
   label=reachability_label(stage=stage,successes=successes,branches=a.branches,branch_records=branch_records,controller_bank_exhausted=True);label.update({"candidate_id":row['id'],"state_byte_hash":hashlib.sha256(b''.join(np.ascontiguousarray(np.asarray(row[k],np.float32)).tobytes() for k in ('qpos','qvel','ctrl','qacc_warmstart'))).hexdigest(),"controller_bank":[file_sha256(Path(policy)/'params.pkl')],"reference_index":row.get('reference_index'),"trajectory_parent":row.get('trajectory_parent_id',row.get('parent_candidate_id'))});labels.append(label)
 ...
 save_json(a.output,{"status":"PASS","artifact_role":"stage_candidate_label_pilot","protocol_sha256":protocol['protocol_sha256'],"states_per_stage":a.states_per_stage,"branches_per_state":a.branches,"horizon":a.horizon,"unique_states":len(labels),"total_rollouts":len(labels)*a.branches,"labels":labels,"label_counts":summary,"success_rates":success_rates,"termination_reasons":dict(terminal),"entry_snapshots":len(entries),"entry_bank":str(a.entry_bank),"entry_bank_sha256":file_sha256(a.entry_bank),"proposal_support_only":True})
