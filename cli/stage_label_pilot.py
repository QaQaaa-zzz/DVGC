 ...
 for stage,rows in selected.items():
  ...
  for i,row in enumerate(rows):
   ...
   for branch in range(a.branches):
    seed=STAGE_SEED[stage]+i*100+branch;policy=policies[branch%len(policies)];inference=inferences[branch%len(inferences)];success,tick,snapshot,quality,reason=run_branch(env,step,inference,row,stage,seed,a.horizon,a.action_noise);successes+=int(success);terminal[f'{stage}:{reason}']+=1;entry_id=None
   ...
   label=reachability_label(stage=stage,successes=successes,branches=a.branches,branch_records=branch_records,controller_bank_exhausted=True);label.update({"candidate_id":row['id'],"state_byte_hash":hashlib.sha256(b''.join(np.ascontiguousarray(np.asarray(row[k],np.float32)).tobytes() for k in ('qpos','qvel','ctrl','qacc_warmstart'))).hexdigest(),"controller_bank":[file_sha256(Path(policy)/'params.pkl') for policy in policies],"reference_index":row.get('reference_index'),"trajectory_parent":row.get('trajectory_parent_id',row.get('parent_candidate_id'))});labels.append(label)
