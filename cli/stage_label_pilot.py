...
def main():
  ...
  for i,row in enumerate(rows):
   ...
   for policy_index,(policy,inference) in enumerate(zip(policies,inferences)):
    for branch in range(a.branches):
     seed=(a.seed_base if a.seed_base is not None else STAGE_SEED[stage])+policy_index*1_000_000+i*100+branch;success,tick,snapshot,quality,reason=run_branch(env,step,inference,row,stage,seed,a.horizon,a.action_noise,support_metadata);successes+=int(success);terminal[f'{stage}:{reason}']+=1;entry_id=None;controller_hash=file_sha256(Path(policy)/'params.pkl')
 ...
 save_json(a.output,{"status":"PASS","artifact_role":"stage_candidate_label_pilot","labeler_version":4,"protocol_sha256":protocol['protocol_sha256'],"seed_base":a.seed_base,"states_per_stage":a.states_per_stage,"branches_per_controller_state":a.branches,"horizon":a.horizon,"unique_states":len(labels),"total_rollouts":sum(x['n'] for x in labels),"labels":labels,"label_counts":summary,"success_rates":success_rates,"termination_reasons":dict(terminal),"entry_snapshots":len(entries),"entry_bank":str(a.entry_bank),"entry_bank_sha256":file_sha256(a.entry_bank),"proposal_support_only":True})
