...
def main():
    ...
    flags={"all_finite":all(np.isfinite(np.asarray(r["qpos"])).all() and np.isfinite(np.asarray(r["qvel"])).all() for r in rows),"all_flight_phase":all(int(r.get("oracle_phase",-1))==2 for r in rows),"children_state_unique":len(child_ids)==len(set(child_ids)) and not (set(child_ids)&set(base_ids)),"children_no_contact":child_contact["records_with_robot_terrain_contact"]==0,"children_no_deep_penetration":child_contact["records_with_deep_penetration"]==0,"children_short_physical_failure_zero":child_rollout["short_horizon_physical_failure_rate"]==0,"children_nonfinite_zero":child_rollout["nonfinite_records"]==0,"parent_cap":max(parent_counts.values(),default=0)<=int(cfg.descent_local_max_children_per_parent),"group_support":all(groups[g]>0 for g in ("provisional_safe","boundary","successful_anchor")),"provenance_current":bank.metadata.get("xml_sha256")==file_sha256(cfg.xml_path) and bank.metadata.get("candidate_config_hash")==config_hash(cfg)}
