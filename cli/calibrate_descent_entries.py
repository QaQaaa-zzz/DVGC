...
def main():
    ...
    cfg=load_config(a.config); bank=SnapshotBank.load(a.certified_bank); rows=bank.records_for_phase("flight",include_training_only=False); safe=[r for r in rows if r["final"]["label"]=="safe"]
    ...
    result=copy.deepcopy(bank); result.metadata.update({"entry_bank_role":"canonical_descent_handoff_set","entry_set_version":bank.metadata["last_tube_version"],"entry_matcher":matcher}); result.save(out)
    report={"status":"PASS" if independent["precision"]>=float(cfg.descent_entry_minimum_calibration_precision) else "FAIL","states":len(rows),"safe_entries":len(safe),"labels":{k:labels.count(k) for k in ("safe","boundary","dead","unknown")},"radius":cal["radius"],"calibration_precision":cal["precision"],"calibration_recall":cal["recall"],"independent_audit":independent,"bank_sha256":file_sha256(out)}; out.with_suffix(".calibration.json").write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2))
