...
def main():
    ...
    layers=difficulty_layers(rows,distances); records=[]; parents={group:[] for group in BOOTSTRAP_GROUPS}
    for original in rows:
        ...
        row.update({"candidate_kind":"descent_diagnostic_anchor","old_policy_label":label,"old_policy_hash":source.metadata["descent_policy_hash"],"bootstrap_group":group,"local_bootstrap_eligible":group in BOOTSTRAP_GROUPS,"descent_layer":layers[row["id"]],"distance_to_c_l":distances[row["id"]],"reset_source":"descent_local_candidate"}); records.append(row)
        if group in parents: parents[group].append(row)
    ...
    for identifier in successful_ids:
        ...
        row=copy.deepcopy(flight_by[identifier]); state=restore_snapshot(env,row,jax.random.PRNGKey(a.seed+len(records))); distance=matcher.match(state)[1]; row.update({"candidate_kind":"descent_successful_anchor","old_policy_label":"successful_rollout_anchor","old_policy_hash":source.metadata["descent_policy_hash"],"bootstrap_group":"successful_anchor","local_bootstrap_eligible":True,"descent_layer":"late","distance_to_c_l":distance,"reset_source":"descent_local_candidate","entry_source_id":identifier,"proposal_step":0}); records.append(row); parents["successful_anchor"].append(row)
    ...
    SnapshotBank(records,metadata).save(out); parent_counts=Counter(row.get("parent_candidate_id") for row in records if row["candidate_kind"]=="descent_local_proposal"); source_counts=Counter(row.get("entry_source_id") for row in records if row.get("local_bootstrap_eligible"))
    report={"status":"PASS" if sum(accepted.values())==int(cfg.descent_local_target_children) else "FAIL","records":len(records),"diagnostic_records":sum(not row.get("local_bootstrap_eligible") for row in records),"eligible_records":sum(bool(row.get("local_bootstrap_eligible")) for row in records),"base_groups":{g:sum(row["bootstrap_group"]==g and row["candidate_kind"]!="descent_local_proposal" for row in records) for g in BOOTSTRAP_GROUPS},"accepted_children":dict(accepted),"targets":targets,"attempts":attempts,"rejections":dict(rejected),"layers":dict(Counter(row["descent_layer"] for row in records if row.get("local_bootstrap_eligible"))),"unique_parent_candidates":len(parent_counts),"maximum_children_per_parent":max(parent_counts.values(),default=0),"unique_source_parents":len(source_counts),"maximum_records_per_source":max(source_counts.values(),default=0),"bank_sha256":file_sha256(out),"provenance":{"source_bank_sha256":file_sha256(a.certified_candidates),"flight_bank_sha256":file_sha256(a.flight_bank),"c_l_sha256":file_sha256(a.landing_entry_set),"xml_sha256":file_sha256(cfg.xml_path),"seed":a.seed}}
