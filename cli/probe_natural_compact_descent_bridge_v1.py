...
def main():
    ...
    reference=pd.read_csv("data/reference_jump.csv");tube_features=np.asarray([r["entry_feature"] for r in tube.records],float)
    ...
    def run(switch_tick):
        ...
        for tick in range(220):
            ...
            feature=descent_entry_feature(physical,cfg);distance,index,contribution=normalized_nearest(feature,tube_features,center,scale)
            ...
            trace.append({"tick":tick+1,"controller":controller,"distance":distance,"phase":int(state.info["phase"]),
        ...
        return {"switch_tick":switch_tick,"approach_ticks":approach,"apex_tick":apex,"closest":closest,"stable_descent_ticks":stable,
            ...
            "termination_reason":END_REASON.get(int(state.info["end_code"]),"unknown"),"steps":len(trace),"trace":trace}

    root.mkdir(parents=True);inputs={"tube_sha256":file_sha256(TUBE),"adapter_sha256":file_sha256(EXPERT/"adapter.pkl"),
    ...
    save_json(root/"manifest.json",{"status":"FROZEN_BEFORE_OUTCOMES","inputs":inputs,"reference_offset":105,"reference_stride":10,
        "handoff_offsets":[-8,-6,-4,-2,0,2,4],"selection":"nominal closest post-apex tick; fixed offsets","matcher_active":False})
    ...
    report={"status":"PASS" if final else "FAIL","head":subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),
        ...
        "maximum_stable_descent_ticks":max(row["stable_descent_ticks"] for row in rows),"termination_reasons":{reason:sum(r["termination_reason"]==reason for r in rows) for reason in sorted({r["termination_reason"] for r in rows})},
